...
def evaluate_classifier(
    ...
    *,
    ...
    # NOTE: overall accuracy slightly differs from original model.evaluate method

    assert y_test_flat.shape == y_pred.shape, (
        f"y_test and y_pred don't have the same shape: y_test: \
        {y_test_flat.shape} != y_pred: {y_pred.shape}. proposed fix: flatten y_test in image dataloading"
    )
...
def main(dataset: str = DATASET, stage: str = "final"):
    ...
    for model_index, (weights_row, output_row, config_row) in tqdm(
        enumerate(zip(weights, outputs, configs.itertuples())), total=len(weights), desc="Evaluating models"
    ):
