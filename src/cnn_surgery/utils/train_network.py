...
flags.DEFINE_string("activation", "relu", "Nonlinear activation: relu / tanh / sigmoind / selu.")
flags.DEFINE_string("w_init", "he_normal", "Initialization for weights. see tf.keras.initializers for options")
flags.DEFINE_string("b_init", "zero", "Initialization for biases.see tf.keras.initializers for options")
...
def build_fcn(n_layers, n_hidden, n_outputs, dropout_rate, activation, w_regularizer, w_init, b_init, use_batchnorm):
...
def eval_model(model, data_tr, data_te, info, logger, cur_epoch, workdir):
    ...
    # Map the metrics based on available keys
    loss_key = "loss"
    acc_key = "compile_metrics"  # This contains the accuracy metric

    metrics = {
        "train_accuracy": res_tr[acc_key] if acc_key in res_tr else 0.0,
        "train_loss": res_tr[loss_key],
        "test_accuracy": res_te[acc_key] if acc_key in res_te else 0.0,
        "test_loss": res_te[loss_key],
    }
    for k in metrics:
...
def run(
    ...
    with strategy.scope():  # type: ignore
        ...
        if architecture == "cnn" or architecture == "cnnbn":
            ...
            model = build_fcn(n_layers, n_hiddens, n_outputs, dropout_rate, activation, w_reg, w_init, b_init, False)
        else:
    ...
    ckpt = tf.train.Checkpoint(step=optimizer.iterations, optimizer=optimizer, model=model)
...
def main(unused_argv):
    ...
    if FLAGS.use_tpu:
        logging.info("Use TPU at %s with job name '%s'.", FLAGS.master, FLAGS.tpu_job_name)
        resolver = tf.distribute.cluster_resolver.TPUClusterResolver(tpu=FLAGS.master, job_name=FLAGS.tpu_job_name)
        if use_remote_eager:
        ...
        if use_remote_eager:
            tf.config.experimental_connect_to_host(FLAGS.master, job_name="gpu_worker")
            logging.warning("Remote eager configured. Remote eager can be slow.")
